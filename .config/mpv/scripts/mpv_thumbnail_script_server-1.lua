...
function create_thumbnail_mpv(file_path, timestamp, size, output_path, options)
	...
	local mpv_command = skip_nil({
		...
		(
			options.relative_scale == nil and ("--vf=scale=%d:%d"):format(size.w, size.h)
...
function create_thumbnail_ffmpeg(file_path, timestamp, size, output_path, options)
	...
	local ffmpeg_command = {
		...
		"-vf",
		(options.relative_scale == nil and ("scale=%d:%d"):format(size.w, size.h) or ("scale=iw*%d:ih*%d"):format(
			size.w,
			size.h
		)),
	...
	local generate_thumbnail_for_index = function(thumbnail_index)
		...
		if need_thumbnail_generation then
			...
			if thumb_state.storyboard ~= nil then
				...
				if url == nil then
					url = thumb_state.storyboard.fragment_base_url
						.. "/"
						.. thumb_state.storyboard.fragments[atlas_idx + 1].path
				end
				local ret = thumbnail_func(
					url,
					0,
					{ w = thumb_state.storyboard.scale, h = thumb_state.storyboard.scale },
					atlas_path,
					{ relative_scale = true }
				)
